...
$oRepuestos = getArray($query);
// If there are repuestos, display them in a table
if ($oRepuestos != null && !empty($oRepuestos)) {
                    ...
                    <div class='col-md-12'>
                        <div class='card mb-4'>
                            <div class='card-header d-flex justify-content-between align-items-center'>
                                <h4 class='text-center'>Repuestos</h4>
                            ...
                            <div class='card-body cardAdmin'>
                                <div class='table-responsive'>
                                    <table class='table table-striped'>
                                        ...
                                        <tbody>";
    // Loop through each repuesto and create a row for it
    foreach ($oRepuestos as $value) {
        echo "<tr>
                ...
                <td>
                    <button id=eliminar class='btn btn-danger' onclick='eliminarRepuesto({$value['id_repuesto']})'>
                        <i class='fas fa-trash'></i> Eliminar
                    </button>
                    <a href='#' class='btn btn-success' onclick='cargarFormularioActualizacion({$value['id_repuesto']})' data-bs-toggle='modal' data-bs-target='#modalUpdate' data-bs-whatever='@mdo'>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #475A68;">
                <h5 style="margin-left: 300px">Agregar repuesto</h5>
...
<script>
    function cargarFormularioActualizacion(id) {
        // Hacer una solicitud AJAX para obtener los detalles del repuesto a actualizar
        $.ajax({
            type: "POST",
            url: "../DAL/repuestos.php", // Ruta al archivo PHP que manejará la solicitud
            data: {
                id: id,
                action: 'obtenerDetalles'
            }, // Pasamos el ID y la acción para obtener los detalles del repuesto
            success: function(response) {
                // Cargar los detalles del repuesto en el formulario de actualización modal
                var data = JSON.parse(response);
                // Asegurarse de que el formulario en modalUpdate se rellena correctamente:
                    $('#modalUpdate [name="id_repuesto"]').val(data.id_repuesto);
    ...
    function actualizarRepuesto() {
        ...
        // Hacer una solicitud AJAX para actualizar el repuesto
        $.ajax({
            type: "POST",
            url: "../DAL/repuestos.php", // Ruta al archivo PHP que manejará la actualización
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                // Recargar la página o actualizar la tabla de repuestos
                location.reload();
            },
            error: function(xhr, status, error) {
                // Maneja errores
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $dbRepuesto = IngresarRepuesto($categoria, $descripcion, $detalle, $talla, $precio, $existencias, $img, $activo);

    if ($dbRepuesto) {
        echo "<script>
                function mostrarMensajeModal() {
                    // Mostrar el mensaje en el modal
                    document.getElementById('messageText').innerHTML = 'Nuevo repuesto ha sido agregado';
...
<script>
    function eliminarRepuesto(id) {
        if (confirm("¿Estás seguro de que deseas eliminar este repuesto?")) {
            // Envía una solicitud AJAX al backend para eliminar el repuesto
            $.ajax({
                type: "POST",
                url: "../DAL/repuestos.php", // Ruta al archivo PHP que contiene la función de eliminación
                data: {
                    action: 'eliminar',
                    id: id
                }, // Pasamos el ID y la acción a realizar
                success: function(response) {
                    // Recarga la página o actualiza la tabla de repuestos
                    location.reload();
                },
                error: function(xhr, status, error) {
                    // Maneja errores
