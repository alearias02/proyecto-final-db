...
$query = "select * from camisa";
$oCamisas = getArray($query);
// If there are camisas, display them in a table
if ($oCamisas != null && !empty($oCamisas)) {
                    ...
                    <div class='col-md-12'>
                        <div class='card mb-4'>
                            <div class='card-header d-flex justify-content-between align-items-center'>
                                <h4 class='text-center'>Camisas</h4>
    ...
    foreach ($oCamisas as $value) {
        echo "<tr>
                <td>{$value['id_camisa']}</td>
                ...
                <td>
                    <button id=eliminar class='btn btn-danger' onclick='eliminarCamisa({$value['id_camisa']})'>
                        <i class='fas fa-trash'></i> Eliminar
                    </button>
                    <a href='#' class='btn btn-success' onclick='cargarFormularioActualizacion({$value['id_camisa']})' data-bs-toggle='modal' data-bs-target='#modalUpdate' data-bs-whatever='@mdo'>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header text-white " style="background-color: #475A68;">
                <h5 style="margin-left: 300px">Agregar camisa</h5>
            ...
            <form method="POST" class="was-validated " enctype="multipart/form-data">
                <div class="modal-body text-center" style="background-color: #eee;">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control mt-2 " style="border: none; box-shadow: 5px 5px 2px 2px rgba(0, 0, 0,.2);" name="categoria" id="categoria" value="6" />
                    </div>
                    <div class="mb-3">
...
<div id="modalUpdate" class="modal fade" tabindex="-1" aria-labelledby="modalUpdateLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: #475A68;">
                <h5 style="margin-left: 300px">Actualizar camisa</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="formUpdate" method="POST" class="was-validated" enctype="multipart/form-data">
                <div class="modal-body text-center" style="background-color: #eee;">
                    <input type="hidden" name="id_camisa" id="id_camisa" value='" .  intval($camisa[' id_camisa']) . "'> 
                    <div class='mb-3'>
                        <label for='descripcion'>Descripción</label>
                        <input type='text' class='form-control mt-2' style='border: none; box-shadow: 5px 5px 2px 2px rgba(0, 0, 0,.2);' name='descripcion' required='true' value='" . htmlspecialchars($camisa[' descripcion']) . "' />
                    </div>
                    <div class='mb-3'>
                        <label for='detalle'>Detalle</label>
                        <input type='text' class='form-control mt-2' style='border: none; box-shadow: 5px 5px 2px 2px rgba(0, 0, 0,.2);' name='detalle' required='true' value='" . htmlspecialchars($camisa['detalle']) . "' />
                    </div>
                    <div class=" mb-3">
                        <label for="talla">Talla</label>
                        <input type="text" class="form-control mt-2 " style="border: none; box-shadow: 5px 5px 2px 2px rgba(0, 0, 0,.2);" name="talla" required="true" value='" . htmlspecialchars($camisa[' talla']) . "'  />
                    </div>
                    <div class=" mb-3">
                        <label for="precio">Precio</label>
                        <input type="text" class="form-control mt-2 " style="border: none; box-shadow: 5px 5px 2px 2px rgba(0, 0, 0,.2);" name="precio" required="true" value='" . floatval($camisa[' precio']) . "' />
                    </div>
                    <div class=" mb-3">
                        <label for="existencias">Existencias</label>
                        <input type="text" class="form-control mt-2 " style="border: none; box-shadow: 5px 5px 2px 2px rgba(0, 0, 0,.2);" name="existencias" required="true" value='" . intval($camisa[' existencias']) . "' />
                    </div>
                    <div class="mb-3">
                        <label for="activo">Activo</label>
                        <input class="form-check-input" type="checkbox" style="margin-top: 0.5rem;" name="activo" id="activoCheckbox" 
                            <?php echo (isset($camisa) && $camisa['activo'] == 1) ? 'checked' : ''; ?> />
                    </div>
                    <div class="mb-3">
                        <label for="categoria">Categoria</label>
                        <input type="text" class="form-control mt-2 " style="border: none; box-shadow: 5px 5px 2px 2px rgba(0, 0, 0,.2);" name="categoria" id="categoria" value='" . intval($camisa[' existencias']) . "' />
                    </div>
                    <div class="mb-3">
                        <label for="imagen">Imagen del item</label>
                        ...
                        <img id="imagenDisplay" src="#" alt="Imagen del Camisa" style="height: 200px;" />
                    </div>
                </div>
                <div class="modal-footer justify-content-center">
...
<script>
    function cargarFormularioActualizacion(id) {
        // Hacer una solicitud AJAX para obtener los detalles del camisa a actualizar
        $.ajax({
            type: "POST",
            url: "../DAL/camisas.php", // Ruta al archivo PHP que manejará la solicitud
            data: {
                id: id,
                action: 'obtenerDetalles'
            }, // Pasamos el ID y la acción para obtener los detalles del camisa
            success: function(response) {
                // Cargar los detalles del camisa en el formulario de actualización modal
                var data = JSON.parse(response);
                // Asegurarse de que el formulario en modalUpdate se rellena correctamente:
                    $('#modalUpdate [name="id_camisa"]').val(data.id_camisa);
    ...
    function actualizarCamisa() {
        // Obtener los valores del formulario de actualización
        var formData = new FormData($('#formUpdate')[0]);

        // Hacer una solicitud AJAX para actualizar el camisa
        $.ajax({
            type: "POST",
            url: "../DAL/camisas.php", // Ruta al archivo PHP que manejará la actualización
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                // Recargar la página o actualizar la tabla de camisas
                location.reload();
            },
            error: function(xhr, status, error) {
                // Maneja errores
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $dbCamisa = IngresarCamisa($categoria, $descripcion, $detalle, $talla, $precio, $existencias, $img, $activo);

    if ($dbCamisa) {
        echo "<script>
                function mostrarMensajeModal() {
                    // Mostrar el mensaje en el modal
                    document.getElementById('messageText').innerHTML = 'Nuevo camisa ha sido agregado';
...
<script>
    function eliminarCamisa(id) {
        if (confirm("¿Estás seguro de que deseas eliminar este camisa?")) {
            // Envía una solicitud AJAX al backend para eliminar el camisa
            $.ajax({
                type: "POST",
                url: "../DAL/camisas.php", // Ruta al archivo PHP que contiene la función de eliminación
                data: {
                    action: 'eliminar',
                    id: id
                }, // Pasamos el ID y la acción a realizar
                success: function(response) {
                    // Recarga la página o actualiza la tabla de camisas
                    location.reload();
                },
                error: function(xhr, status, error) {
                    // Maneja errores
